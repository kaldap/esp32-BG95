...
bool MODEMBGXX::wait_modem_to_soft_init()
{
    if(wait_command("APP RDY", 10000))
    {
        increase_modem_reboot();
...
// count += tcp_recv(clientID,&data[count],len-count);
            ...
            ESP_LOGD(TAG, "bytes available on buffer %d \n", count);
#endif
            return count;
        }
...
bool MODEMBGXX::enable_psm(uint8_t tau, uint8_t active_time)
{
    String s = "AT+CPSMS=1,,,\"" + String(tau) + "\",\"" + String(active_time) + "\"";
    return check_command(s.c_str(), "OK", 2000);
}

bool MODEMBGXX::disable_psm()
{
    String s = "AT+CPSMS=0";
    return check_command(s.c_str(), "OK", 2000);
}